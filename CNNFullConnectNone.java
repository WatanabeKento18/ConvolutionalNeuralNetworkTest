...
public class CNNFullConnectNone extends CNNBaseLayerFullConnect{
	private float[][][] beforeError;	//直前の誤差
	
	//sizeNin:入力ノード数 sizeNout:出力ノード数 iSizeX:入力データ横サイズ iSizeY:入力データ縦サイズ
	public CNNFullConnectNone(int sizeNin , int iSizeX , int iSizeY, int sizeNout ){
		this.layerCode = 1043;
			...
			for(int i = 0 ; i < iSizeX ; i++){
				for(int j = 0 ; j < iSizeY ; j++){
					for(int no = 0 ; no < sizeNout ; no++){
						w[ni][i][j][no] = (rnd.nextFloat() - 0.5f)*0.1f;
					}
				}				
			}			
		}
	...
	public float[][][] backPropagation(float[][][] output , float[] label){
		...
		for(no = 0 ; no < sizeNout ; no++){
			...
			for(ni = 0 ; ni < sizeNin ; ni++){
				for(i = 0 ; i < iSizeX ; i++){
					for(j = 0 ; j < iSizeY ; j++){
						totalError = d[no] * beforeInput[ni][i][j];
	...
	public static void main(String[] args){
		...
		/*CNNFullConnectNone fcl = new CNNFullConnectNone(1,3,3,7);
		CNNFullConnectNone fcl2 = new CNNFullConnectNone(7,1,1,4);
		CNNFullConnectNone fcl3 = new CNNFullConnectNone(4,1,1,2)*/;
		CNNFullConnectNone fcl = new CNNFullConnectNone(1,3,3,2);
